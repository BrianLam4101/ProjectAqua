...
public class waterFountain : Appliance {
    ...
    void Start() {
        ...
        faucet = transform.parent.Find("Tube");
        ...
        if (on >= 1) {
            if (waterHolder.wasteWater(waterUsage * Time.fixedDeltaTime)) {
                ...
                if (Vector3.Distance(transform.position, GameObject.Find("Player").transform.position) < 5) {
                    waterHolder.thirst += thirstUp * Time.fixedDeltaTime;
                    waterHolder.bladder += bladderUp * Time.fixedDeltaTime;
                }
